...
describe('Testing MongodbDishRepository', () => {
  ...
  test('Should add a new Dish in the database', async () => {
    ...
    const foundDish = await DishCollection.findOne({
      restaurantId: dishes[0].restaurantId,
    });

    expect(foundDish?.name).toBe(dishes[0].name);
    expect(foundDish?.description).toBe(dishes[0].description);
    expect(foundDish?.price).toBe(dishes[0].price);
    expect(foundDish?.restaurantId).toBe(dishes[0].restaurantId);
    expect(foundDish?.imageUrl).toBe(dishes[0].imageUrl);
  });
  ...
  test('Should return all dishes in the database', async () => {
    ...
    // Adding new dishes to database
    await DishCollection.insertOne(MongodbMapper.toMongodbDocument(dishes[0]));
    await DishCollection.insertOne(MongodbMapper.toMongodbDocument(dishes[1]));
  ...
  test('Should update Dish by id', async () => {
    ...
    expect(foundDish?.name).toBe(updatedDish.name);
    expect(foundDish?.description).toBe(updatedDish.description);
    expect(foundDish?.price).toBe(updatedDish.price);
    expect(foundDish?.restaurantId).toBe(updatedDish.restaurantId);
    expect(foundDish?.imageUrl).toBe(updatedDish.imageUrl);
  });
