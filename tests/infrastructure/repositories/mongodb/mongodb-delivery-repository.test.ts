...
describe('Testing MongodbDeliveryRepository', () => {
  ...
  test('Should add a new Delivery in the database', async () => {
    ...
    const foundDelivery = await DeliveryCollection.findOne({
      _id: mongoHelper.toObjectId(deliverys[0].id),
    });

    expect(foundDelivery?.orderId).toBe(deliverys[0].orderId);
    expect(foundDelivery?.driverId).toBe(deliverys[0].driverId);
    expect(foundDelivery?.status).toBe(deliverys[0].status);
    expect(foundDelivery?.timeEstimateInMinutes).toBe(
      deliverys[0].timeEstimateInMinutes
    );
  });
  ...
  test('Should return all deliverys in the database', async () => {
    ...
    await DeliveryCollection.insertOne(
      MongodbMapper.toMongodbDocument(deliverys[0])
    );
    await DeliveryCollection.insertOne(
      MongodbMapper.toMongodbDocument(deliverys[1])
    );
  ...
  test('Should update Delivery by id', async () => {
    ...
    expect(foundDelivery?.orderId).toBe(updatedDelivery.orderId);
    expect(foundDelivery?.driverId).toBe(updatedDelivery.driverId);
    expect(foundDelivery?.status).toBe(updatedDelivery.status);
    expect(foundDelivery?.timeEstimateInMinutes).toBe(
      updatedDelivery.timeEstimateInMinutes
    );
  });
  ...
  test('Should remove Delivery by id', async () => {
    ...
    expect(foundDelivery).toBeFalsy();
  });
});
