...
describe('Testing MongodbRestaurantRepository', () => {
  ...
  test('Should add a new restaurant in the database', async () => {
    ...
    const foundRestaurant = await restaurantCollection.findOne({
      adminId: restaurants[0].adminId,
    });

    expect(foundRestaurant?.restaurantChainId).toBe(
      restaurants[0].restaurantChainId
    );
    expect(foundRestaurant?.adminId).toBe(restaurants[0].adminId);
    expect(foundRestaurant?.isOpen).toBe(restaurants[0].isOpen);
    expect(foundRestaurant?.phone).toBe(restaurants[0].phone);
    expect(foundRestaurant?.imageUrl).toBe(restaurants[0].imageUrl);
    expect(JSON.stringify(foundRestaurant?.address)).toBe(
      JSON.stringify(restaurants[0].address)
    );
  });
  ...
  test('Should return all restaurants in the database', async () => {
    ...
    await restaurantCollection.insertOne(
      MongodbMapper.toMongodbDocument(restaurants[0])
    );
    await restaurantCollection.insertOne(
      MongodbMapper.toMongodbDocument(restaurants[1])
    );

    const allRestaurants = await repository.findAllRestaurants();

    expect(allRestaurants[0].restaurantChainId).toEqual(
      restaurants[0].restaurantChainId
    );
    expect(allRestaurants[1].restaurantChainId).toEqual(
      restaurants[1].restaurantChainId
    );
  });
  ...
  test('Should find a restaurant by id', async () => {
    ...
    const foundRestaurant = await repository.findRestaurantById(
  ...
  test('Should find a restaurant by adminId', async () => {
    ...
    const foundRestaurant = await repository.findRestaurantByAdminId(
      restaurants[0].adminId
    );
  ...
  test('Should update restaurant by id', async () => {
    ...
    expect(foundRestaurant?.restaurantChainId).toBe(
      updatedRestaurant.restaurantChainId
    );
    expect(foundRestaurant?.adminId).toBe(updatedRestaurant.adminId);
    expect(foundRestaurant?.isOpen).toBe(updatedRestaurant.isOpen);
    expect(foundRestaurant?.phone).toBe(updatedRestaurant.phone);
    expect(foundRestaurant?.imageUrl).toBe(updatedRestaurant.imageUrl);
    expect(JSON.stringify(foundRestaurant?.address)).toBe(
      JSON.stringify(updatedRestaurant.address)
    );
  });
  ...
  test('Should remove restaurant by id', async () => {
    ...
    expect(foundRestaurant).toBeFalsy();
  });
});
